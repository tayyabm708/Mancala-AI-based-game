...
class MancalaGUI:
    ...
    def new_game(self):
        ...
        for i in range(7, 14):  # Extra pit for both players
    ...
    def end_game(self):
        player_score = self.game.board[6]
        ai_score = self.game.board[13]
...
class MancalaGame:
    def __init__(self):
        ...
        self.board[6] = 0  # Player's Mancala
        self.board[13] = 0  # AI's Mancala
    ...
    def make_move(self, move):
        ...
        while num_stones > 0:
            ...
            if current_pocket == 6 and self.current_player == 1:
                current_pocket = (current_pocket + 1) % 14
            elif current_pocket == 13 and self.current_player == 2:
                current_pocket = (current_pocket + 1) % 14

            self.board[current_pocket] += 1
            num_stones -= 1

        if current_pocket != 6 and self.board[current_pocket] == 1 and self.board[12 - current_pocket] > 0:
            if self.current_player == 1 and current_pocket in range(6):
                self.board[6] += self.board[current_pocket] + self.board[12 - current_pocket]
                self.board[current_pocket] = 0
                self.board[12 - current_pocket] = 0
            elif self.current_player == 2 and current_pocket in range(7, 13):
                self.board[13] += self.board[current_pocket] + self.board[12 - current_pocket]
                self.board[current_pocket] = 0
                self.board[12 - current_pocket] = 0
